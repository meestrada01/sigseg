...
import { getTipoProducto } from 'store/slices/inventario';
import ModeEditOutlineIcon from '@mui/icons-material/ModeEditOutline';
import CancelIcon from '@mui/icons-material/Cancel';
import DoneOutlineIcon from '@mui/icons-material/DoneOutline';

export function TipoProductoLis({ ItemsTipoProducto }) {
    const dispatch = useDispatch();
    const [rows, setRows] = React.useState([]);

    const { tipoproducto, postipoproducto } = useSelector((state) => state.inventario);

    React.useEffect(() => {
        dispatch(getTipoProducto());
    }, [dispatch, postipoproducto]);

    React.useEffect(() => {
        setRows(tipoproducto);
...
TipoProductoLis.propTypes = {
    ItemsTipoProducto: PropTypes.func
};

export default TipoProductoLis;
